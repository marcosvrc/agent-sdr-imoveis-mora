...
para o formato do canal (teclado inline do Telegram, JSON do WebSocket, texto da CLI). Nunca chamam
o LLM nem leem o cartão do lead. Um lead pode ter N canais (`tabela canais`), com um único histórico.
...
- Migração web → Telegram preserva histórico via `lead_id`.
