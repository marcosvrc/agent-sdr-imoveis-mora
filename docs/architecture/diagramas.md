...
![Topologia do docker compose](../assets/diagramas/topologia-claro.svg#only-light)
![Topologia do docker compose](../assets/diagramas/topologia-escuro.svg#only-dark)
...
sobre um sistema visual comum (`base.py`), e sai em dois arquivos — claro e escuro — sob
`docs/assets/diagramas/`.

```bash
make diagramas    # regera os 20 arquivos (10 diagramas × 2 temas)
```
...
declarar a posição; em troca, o conteúdo fica versionado como código, o tema escuro acompanha o do
portal (`#only-light` / `#only-dark`) e o GitHub renderiza o mesmo desenho pelo `<picture>` do
README. Um teste percorre cada SVG no navegador e reprova texto que transborde da caixa que o
contém.
